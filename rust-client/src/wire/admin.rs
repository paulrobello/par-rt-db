...
// `Cast` / `ValueExpr` / `CaseWhen` used to live here (the migrate-path
// spelling). ENH-028 gave the grammar a second consumer — the schema's
// `computed` map — which compiles without the `admin` feature, so the
// canonical definitions moved to the unconditional `crate::value_expr`
// (mirroring server `src/value_expr.rs`'s home) and are re-exported here so
// `wire::admin::ValueExpr` keeps naming the same, wire-identical type.
