...
- [x] **Fine-grained subscription invalidation: range/boundary tracking** — extend today's `get(id)` point-read skipping (FEATURE_MATRIX #21) to ordered/range/set shapes so subscriptions re-run only when a written doc actually falls in their bounds; spec at `docs/superpowers/specs/2026-07-24-fine-grained-subscription-invalidation-design.md`. (effort: large) — **v2 SHIPPED (2026-07-28):** `count`/`collect`/`unique` on a btree index's eq-prefix (+ optional range bound) now skip re-runs when every written doc is provably outside the window (`WriteSet.doc_values` carries before/after states; `ReadSet::Indexed` evaluates `in_window` per written doc; never under-approximates — deletes always re-run, any doubt over-approximates). `take`/`first`/`paginate`/`distinct`/`aggregate`/`search`/`vector` stay table-level. **Ordered top-N boundary tracking (the dynamic Nth-value window) remains deferred** — the hardest case (a doc can enter/leave the truncated window even when its eq-prefix is unchanged) and the one piece the v2 plumbing does not yet cover soundly.
