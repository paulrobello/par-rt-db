...
This is a three-package workspace: the Rust **server** in `server/`, the **TypeScript client SDK** (`@par-rt-db/client`) in `client/`, and the **Rust client crate** (`par-rt-db-client`) in `rust-client/`. The **root `Makefile` runs all three** — every target (`build`, `fmt`, `fmt-check`, `lint`, `typecheck`, `test`, `checkall`) `cd`s into `server/` (cargo), `client/` (bun), and `rust-client/` (cargo). Run `cargo` from `server/` or `rust-client/`, and `bun` from `client/`, when invoking a tool directly.
