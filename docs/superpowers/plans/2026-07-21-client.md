...
- **Transaction JSON**: `{steps: Step[]}`. Each `Step` is tagged by **`op`** and **every step carries `table`**: `{op:"insert", table, doc}`, `{op:"patch", table, id, fields}`, `{op:"delete", table, id}`, `{op:"expectVersion", table, id, version}`, `{op:"expectAbsent", table, index, eq}`, `{op:"upsert", table, index, eq, insert, patch}`. Mutate returns `results: unknown[]` — one per step. **Verified against the server (`txn.rs`):** an `insert` step's result is **`{"id": "<generated id>"}`** (an object, not a bare string); `upsert` → `{"id", "inserted": boolean}`; `patch`/`delete`/`expectVersion`/`expectAbsent` → `null`.
...
**Files:**
- Create: `client/package.json`, `client/tsconfig.json`, `client/tsconfig.build.json`, `client/biome.json`, `client/.gitignore` (biome's `useIgnoreFile: true` errors if no `.gitignore` sits beside `biome.json`; content: `dist/` and `node_modules/`), `client/vitest.config.ts`, `client/Makefile`, `client/src/index.ts`, `client/tests/smoke.test.ts`
...
> **AMENDMENT (post-review correction).** The reference `client.ts` originally inlined below had six defects in paths its six tests never exercised; the adversarial task review caught them and they were fixed in a follow-up commit. **The committed `client/src/client.ts` and `client/tests/client.test.ts` (12 tests) are the authoritative Task 6 deliverable and supersede the inline code below.** A re-run must reproduce these corrections:
> 1. **Valid close codes.** `socket.close(1006, …)` throws `InvalidAccessError` on a real WebSocket (1006 is reserved). Use an app-range code (`CLOSE_APP_DROP = 4000`) for the heartbeat-timeout and `onerror` force-closes; wrap `detachSocket`'s `close()` in try/catch.
> 2. **Terminal teardown settles all mutations.** `close()` sets `stopped`, rejects **both** in-flight and never-sent mutations (`rejectAllMutates`), resets `authState` to `unauthenticated`; `mutate()` rejects immediately when `stopped`.
> 3. **Generation counter.** Every open/teardown bumps `this.generation`; async `getToken` resolutions and reconnect-timer callbacks capture their generation and abort if it advanced — so a stale reconnect or late token can't open a duplicate socket. `openSocket()` is the single entry point (clears timers, detaches any socket, bumps generation).
> 4. **`subscribeErr` clears BOTH maps** (`subsById` + `subsByKey`) so the failed query is neither resent on reconnect nor left routing updates.
> 5. **`connState` transitions** on the 4401 and no-token paths set `"idle"` so an explicit `connect()` (e.g. after re-login) can revive; `handleClose` sets `"closed"` when stopped.
> 6. **`setToken(null)` truly signs out** via a `hasToken` flag (so `openSocket` uses the explicit null token instead of re-invoking a caching `getToken`); `flushOnAuth` runs before the `authenticated` auth-state notification to avoid a duplicate subscribe frame; heartbeat force-close threshold is `>=` (two missed intervals).
>
> Fixed in commit recorded in the ledger; 12 tests now cover close/setToken/heartbeat/disconnect-rejection/sign-out/4401-timer paths.
...
> **AMENDMENT (found during the real-server pass).** Two plan bugs surfaced and were fixed:
> 1. **vitest config split.** A global `exclude: ["tests/integration/**"]` in `vitest.config.ts` made `bun run test:integration` (which targets that dir positionally) match **zero** files. Fix: remove `tests/integration/**` from the config `exclude`; the default `test` script excludes it via CLI (`vitest run --exclude 'tests/integration/**'`), and `test:integration` targets the directory directly.
> 2. **Integration files must pin the Node environment.** The default `happy-dom` environment's `fetch` cannot make real network calls, so `tests/integration/e2e.test.ts` must start with `// @vitest-environment node` (real `fetch`/`WebSocket`).
>
> Also: an `insert` step's mutate result is **`{ id }`** (object), not a bare string — the e2e assertion reads `insertResult.id`. Real-server pass verified green (2/2) against a local server on port 8399.
