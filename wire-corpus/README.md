...
| `dbReadOnly` | no | `true` arms the per-database read-only freeze AFTER seeding (the seeds are pre-freeze writes; the op runs against the frozen engine). While frozen, every txn fails with `READ_ONLY` while queries still execute — each runner models the freeze the same way (the server runner flips the real `rtdb_auth.databases.read_only` flag and consults the same `db::is_read_only` gate helper the committer's Mutate arm consults). Transport-level gating is pinned by `server/tests/read_only_test.rs`, not the corpus. |
