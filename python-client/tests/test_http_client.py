...
# --- data plane: mutate batch ------------------------------------------------


def test_mutate_batch_parses_aligned_outcomes() -> None:
    captured: dict[str, Any] = {}
    ...
    out = client.mutate_batch([txn, txn], idempotency_keys=["k1", None])
    ...
    # body shape: {db, txns: [{txn, idempotencyKey?}, ...]}
    assert captured["body"]["db"] == DB
...
def test_schedule_manage_ops_post_to_id_paths() -> None:
    ...
    client = _client(handler)
    assert client.cancel_schedule("sch-1") is True
    assert client.pause_schedule("sch-1") is True
    assert client.resume_schedule("sch-1") is True
    assert seen == [
...
def test_schedule_manage_op_no_op_resolves_false() -> None:
    ...
    client = _client(lambda r: httpx.Response(200, json={"ok": False}))
    assert client.cancel_schedule("sch-gone") is False
    assert client.pause_schedule("sch-gone") is False
    assert client.resume_schedule("sch-gone") is False


def test_schedule_manage_op_error_envelope_raises() -> None:
    client = _client(
...
def test_claim_schedules_posts_db_limit_leasems_and_decodes_jobs() -> None:
    captured: dict[str, Any] = {}
    ...
    client = _client(handler)
    jobs = client.claim_schedules(limit=2, lease_ms=30_000)
...
def test_claim_schedules_body_omits_unset_options() -> None:
    captured: dict[str, Any] = {}
    ...
    client = _client(handler)
    assert client.claim_schedules() == []
    assert captured["body"] == {"db": DB}


@pytest.mark.parametrize("op", ["complete", "retry", "fail"])
def test_finalize_ops_post_to_id_paths(op: str) -> None:
    captured: dict[str, Any] = {}
    ...
    client = _client(handler)
    if op == "complete":
        ok = client.complete_schedule("job-1", lease=3)
    elif op == "retry":
        ok = client.retry_schedule("job-1", lease=3, delay_ms=5_000, error="flaky")
    else:
        ok = client.fail_schedule("job-1", lease=3, error="boom")
    assert ok is True
...
def test_finalize_op_stale_lease_conflict_raises() -> None:
    ...
    with pytest.raises(RtDbError) as ei:
        client.complete_schedule("job-1", lease=1)
