...
| `GET /admin/presence` | Bearer admin key | Live per-room presence inspector: one row per room with member count, serialized-state bytes, and the oldest member's join age. Per-replica (in-memory); in multi-instance mode each replica reports its own rooms. Also surfaced as `presenceDetail` on `GET /admin/metrics`. |
