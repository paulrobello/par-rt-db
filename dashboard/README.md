...
| **Workflows** | `/workflows` | Durable workflow runs across databases — per-step timeline, status filters (incl. `stuck`: terminal `failed` or a non-terminal run burning retries), cancel, and start-from-spec. Auto-refreshes on step boundaries. |
