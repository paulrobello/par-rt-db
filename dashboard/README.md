...
a database. Twenty-one routes mirror the admin API surface:
...
| **Workflows** | `/workflows` | Durable workflow runs across databases — per-step timeline, status filters (incl. `waiting` and `stuck`: terminal `failed` or a non-terminal run burning retries), cancel, start-from-spec, and a send-signal form on `waiting` rows (name prefilled from `waitingFor`, optional JSON payload — releases an `awaitSignal` step via the admin signal route). Auto-refreshes on step boundaries. |
