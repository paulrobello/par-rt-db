...
pagination), and **#8** (session-expiry enforcement) are done as of 2026-07-22 — see
their rows above for what shipped. The next highest-value build is **#9/#10
(scheduled/cron txns)**: the sleeper that delivers most of what apps use Convex's
scheduler+actions for, without compromising the no-server-code architecture. Its
prerequisite (#9's scheduled-txn table) is independent of anything already shipped.
After that, **#11 (full-text search)** is a native Postgres fit (`tsvector` + GIN).
