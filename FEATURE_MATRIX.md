...
| 2 | 1 | **`.first()`** terminal | ✅ | ❌ | Med | S | Sugar over `take(1)` returning `doc \| null`. New `first: bool` in `Query` + client terminal. Trivial, closes a visible API hole. |
| 3 | 1 | **`count()`** terminal | 🟡 (needs aggregate component) | ❌ | Med | S | `SELECT count(*)` over the same WHERE the eq-prefix builds. Postgres makes this free where Convex needs a sharded-counter component — a cheap chance to *exceed* Convex. |
