...
export/import, extra validators), tier-2's **#4** (safe retry), **#5** (cursor
pagination), **#8** (session-expiry enforcement), and **#11** (full-text search) are
done as of 2026-07-23 — see their rows above for what shipped. The next highest-value
build is **#9/#10 (scheduled/cron txns)**: the sleeper that delivers most of what apps
use Convex's scheduler+actions for, without compromising the no-server-code architecture.
Its prerequisite (#9's scheduled-txn table) is independent of anything already shipped.
**#11 (full-text search)** shipped as a native Postgres fit (`tsvector` generated column
+ GIN index per declared search index, ranked by `ts_rank` via a `search` query
terminal); the remaining tier-2 builds are **#12** (optimistic updates) and **#14/#15**.
