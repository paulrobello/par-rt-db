...
**Date:** 2026-07-21 (gap matrix last updated 2026-07-31; dimensional + divergence comparison added 2026-07-29)
...
| Schema migration on push | ✅ | 🟡 additive-only | Destructive pushes rejected by design; safe literal-union widening (adding enum variants) is allowed — backward-compatible (old value set ⊆ new), no backfill. Convex allows more (with backfill) |
...
| §1 (schema migration) | Schema changes | additive + migrate/backfill | additive-only; destructive push rejected (safe literal-union widening allowed) | ⚖ |
...
- **Per-row auth (#20)** — opt-in `ownerField`, enforced on every read terminal, every
  ...
  `patch`/`replace`/`delete`/`upsert`-update → `Forbidden`/403; immutable post-insert).
  Collaborator/role (model B), the general predicate DSL (model C), and
  `ExpectVersion`/`ExpectAbsent` are deferred (see the row).
